...
const Services = () => {
  ...
  return (
    <section id="servicos" className="w-full px-4 py-12 text-white">
      <div className="mx-auto max-w-7xl space-y-8">
        {/* Título */}
        <h2 className="text-3xl font-bold text-start text-rose-600">
          Serviços
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 ">
          {services.slice(0, visibleCount).map((service, index) => (
            <Card
              key={index}
              className="bg-zinc-800 text-white border-none pt-0 shadow-[-4px_-4px_10px_rgba(255,255,255,0.1),4px_4px_10px_rgba(0,0,0,0.2)]">
